...
Bevor damit beginnnen, nochmals eine kurze Wiederholung zu `character`-Objekten in R sowie ein neues Konzept.
...
Reguläre Ausdrücke sehen für Laien oft aus wie Kauderwelsch und benötigen etwas Einübungszeit. Mit den Hilfsfunktionen `str_view()` (erste Übereinstimmung) und `str_view_all()` (alle Übereinstimmungen) können wir uns schnell anzeigen lassen, ob ein *RegEx*-Muster in einem String vorkommt oder nicht.
...
- `.`: Alle Zeichen
- `\d`: Alle Ziffern
- `\w`: Alle alphanumerischen Zeichen (Klein- und Großbuchstaben, Ziffern, Unterstrich)
- `\s`: Whitespace (Leerzeichen, Umbrüche)
